...
def _market_specs() -> list[SettingSpec]:
    ...
    return [
        ...
        SettingSpec("market_max_risk_per_trade_pct", env.market_max_risk_per_trade_pct, "float",
                    "Max % of portfolio value lost if a proposed trade's stop is hit. Caps position size.",
                    "market"),
        SettingSpec("market_stop_move_multiple", env.market_stop_move_multiple, "float",
                    "Stop sits this many average daily moves below the entry price.", "market"),
        SettingSpec("market_min_stop_pct", env.market_min_stop_pct, "float",
                    "Floor on the stop distance, so a quiet name does not get a noise-level stop.", "market"),
        SettingSpec("market_max_stop_pct", env.market_max_stop_pct, "float",
                    "Ceiling on the stop distance, so a volatile name does not get an enormous stop.", "market"),
        SettingSpec("market_reward_risk_target", env.market_reward_risk_target, "float",
                    "Target distance as a multiple of the stop distance (2.0 = risk 1 to make 2).", "market"),
